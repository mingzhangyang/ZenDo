...
const CATEGORY_RULES: Array<{ id: CategoryId; cues: Array<[string, number]> }> = [
  {
    id: 'work_execution',
    cues: [
...
function hasCue(normalizedText: string, paddedText: string, tokenSet: Set<string>, cue: string): boolean {
  if (!cue) return false;
...
function scoreByRules(text: string): CategoryScore[] {
